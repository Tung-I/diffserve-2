...
class Controller(controller_pb2_grpc.ControllerServicer):
    def __init__(self, allocationPolicy: AllocationPolicy):
        ...
        if PIPELINE == 'sdturbo':
            ...
            self.apps = [registerApplication('../../traces/apps/sdturbo_sdv15/multi_models.json')]
    ...
    def allocateByMultiCascadeAlg(self, do_static=False):
        ...
        lookup_table = create_lookup_table(model_comb_thres_config, model_fid_config, total_models=len(model_order))
        ...
        if milp_result is not None: 
            ...
            for i in range(num_avail_workers):
                ...
                route_ratio_dict[model] = 1.0
            self.conf_thres = 0.5
            self.router_thres = 0.5
