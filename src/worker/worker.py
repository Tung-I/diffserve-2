        ...
        try:
            if self.modelName == modelName:
                with lock:
                    ...
                    self.writePipe.send(f'{self.infer_level},{self.router_thres},{self.conf_thres},{self.batch_size},{self.is_lightweight}')
            else:
                with lock:
                    self.writePipe.send('LOAD_MODEL')
                    self.writePipe.send(f'{modelName},{schedulerName},{self.infer_level},{self.router_thres},{self.conf_thres},{self.batch_size},{self.is_lightweight},{appID},{task}')
...
def getargs():
    ...
    parser.add_argument('--cascade', '-c', required=True,
                       dest='cascadeExec', choices=['sdturbo', 'sdxs', 'sdxlltn', 'multi'],
