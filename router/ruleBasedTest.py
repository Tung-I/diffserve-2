...
def scale_w_feat(feat_weight, csv_file, save_to, batch_size, light_model, heavy_model, scale_random=False):
...
def main():
    ...
    num_trials = 400
    feat_weight_candidates = np.round(np.random.uniform(low=0.0, high=1.0, size=(num_trials, 8)), 2)
    if not opt.scale_random:
        np.savetxt("feat_weight_candidates.txt", feat_weight_candidates, fmt="%.2f")
    
    csv_file = "prompt_features.csv"
    light_model = "sd35-large-turbo_4"
    heavy_model = "sd35-large_50"

    for i, feat_weight in enumerate(feat_weight_candidates):
        print(f"Current trial: {i+1} -----------------------------------")
        ps = "random" if opt.scale_random else "feat"
        images_save_to = f"/scratch3/workspace/qizhengyang_umass_edu-diffserve/imagenet1k/rule_based_{ps}"
        scale_w_feat(feat_weight, csv_file, images_save_to, batch_size=4, 
